...
class Ambiente:
    ...
    def imprimir_mapa(self, agente_x, agente_y):
        ...
        print("=== Simulador: Agente Baseado em Modelos (Com Memória) ===")
...
class AgenteReativoBaseadoEmModelos:
    def __init__(self):
        ...
        # O "Modelo" ou "Estado Interno": Matriz 20x20 para lembrar por onde já passou e evitar loops[cite: 1]
        self.visitados = [[0 for _ in range(20)] for _ in range(20)]

    def agir(self, ambiente):
        self.passos += 1
        
        # Atualiza a memória marcando a célula atual como visitada
        self.visitados[self.x - 1][self.y - 1] += 1
        
        conteudo_atual = ambiente.get_conteudo(self.x, self.y)

        # Regra 1: Se está sobre um lixo e não carrega nada, pega o lixo
        ...
        # Regra 2: Se está carregando lixo e chegou na lixeira (20,20), solta o lixo
        ...
        # Regra 3: Se vê lixo vizinho e está de mãos vazias, vai até ele
        if self.carga is None:
            ...
            if vizinhos:
                ...
                # Regra do Modelo: Explora as células menos visitadas para evitar redundância[cite: 1]
                self.mover_inteligentemente()
                self.acao_atual = "Consultou a memória: Explorando área menos visitada."
                return

        # Fallback: Se tem carga, vai pra lixeira (20,20)
    ...
    def mover_inteligentemente(self):
        # Avalia os 8 vizinhos possíveis
        opcoes = []
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                if dx == 0 and dy == 0: continue
                ...
                if 1 <= nx <= 20 and 1 <= ny <= 20:
                    # Olha no modelo (memória) quantas vezes já visitou esse vizinho
                    visitas = self.visitados[nx-1][ny-1]
                    opcoes.append({'x': nx, 'y': ny, 'visitas': visitas})
        
        # Descobre qual é o menor número de visitas entre as opções ao redor
        min_visitas = min(opcao['visitas'] for opcao in opcoes)
        
        # Filtra apenas os vizinhos que têm o menor número de visitas (pode haver empate)
        melhores_opcoes = [opc for opc in opcoes if opc['visitas'] == min_visitas]
        
        # Escolhe aleatoriamente entre os "menos visitados" para desempatar
...
if __name__ == "__main__":
    ...
    amb = Ambiente()
    robo = AgenteReativoBaseadoEmModelos()
    
    inicio_tempo = time.perf_counter()
    
    # Roda até limpar todos os lixos
